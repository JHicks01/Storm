...
static inline void list_add(struct list *n, struct list *head)
{
	list_insert(n, head, head->next);
}

static inline void list_add_tail(struct list *n, struct list *head)
{
	list_insert(n, head->prev, head);
}

static inline void list_del_(struct list *prev, struct list *next)
{
	next->prev = prev;
	prev->next = next;
}
